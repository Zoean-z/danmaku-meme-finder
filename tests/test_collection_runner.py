...
from danmaku_meme_finder.database import iso_now
...
def test_import_and_candidate_build_keep_repeated_non_existing_text(tmp_path: Path) -> None:
    ...
    started_at = iso_now() - timedelta(hours=1)
    records = [
        {"ts": (started_at + timedelta(seconds=index)).isoformat(), "roomId": 6657, "uid": str(index), "text": "新梗模板文本"}
        for index in range(3)
    ] + [
        {"ts": (started_at + timedelta(minutes=1, seconds=index)).isoformat(), "roomId": 6657, "uid": str(10 + index), "text": "已有梗"}
        for index in range(3)
    ]
