...
def test_import_jsonl_uses_checkpoint_and_hashes_uid(tmp_path: Path) -> None:
    ...
    with DanmakuDatabase(tmp_path / "danmaku.db") as database:
        ...
        rows = database.conn.execute("SELECT content, user_key, session_id FROM raw_danmaku ORDER BY id").fetchall()
...
def test_import_jsonl_keeps_optional_session_id(tmp_path: Path) -> None:
    source = tmp_path / "live.jsonl"
    checkpoint = tmp_path / "checkpoint.json"
    source.write_text(
        json.dumps({
            "ts": "2026-07-25T12:00:00+08:00", "roomId": 6657,
            "sessionId": "6657-20260725-120000", "uid": "123", "text": "session message",
        }) + "\n",
        encoding="utf-8",
    )
    with DanmakuDatabase(tmp_path / "danmaku.db") as database:
        result = import_jsonl(source, checkpoint, database, "salt")
        stored_session = database.conn.execute("SELECT session_id FROM raw_danmaku").fetchone()[0]

    assert result["imported"] == 1
    assert stored_session == "6657-20260725-120000"
